...
export function Stage({
  onRetryOutline,
  initialRoundtableQuestion,
}: {
  onRetryOutline?: (outlineId: string) => Promise<void>;
  initialRoundtableQuestion?: string | null;
}) {
  const { t } = useI18n();
  const {
    mode,
  ...
  const autoQuestionSentRef = useRef<string | null>(null);
  ...
  // Auto-start roundtable discussion when entering from the Roundtable menu.
  useEffect(() => {
    const question = initialRoundtableQuestion?.trim();
    if (!question) return;
    if (!currentSceneId) return;
    if (autoQuestionSentRef.current === question) return;

    const timer = setTimeout(async () => {
      const chat = chatAreaRef.current;
      if (!chat) return;

      autoQuestionSentRef.current = question;
      chat.switchToTab('chat');
      setIsCueUser(false);
  ...
  }, [currentSceneId, initialRoundtableQuestion]);
