...
function AnswerSheet({
  ...
  phase: Phase;
  resultMap: Record<string, QuestionResult>;
  ...
  return (
    <div className="sticky top-0">
      <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">答题卡</p>
      <div className="grid grid-cols-2 gap-2">
        {questions.map((q, i) => {
          const active = activeQuestionId === q.id;
          const answered = isAnswered(q.id);
          const result = resultMap[q.id];
          const correct = phase === 'reviewing' && result?.status === 'correct';
          const incorrect = phase === 'reviewing' && result?.status === 'incorrect';
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active && 'ring-2 ring-violet-300 dark:ring-violet-700',
                correct && 'bg-emerald-100 dark:bg-emerald-900/40 border-emerald-300',
                incorrect && 'bg-red-100 dark:bg-red-900/30 border-red-300',
                !correct &&
                  !incorrect &&
                  answered &&
...
export function QuizView({ questions, sceneId, sceneTitle }: QuizViewProps) {
  ...
  const safeActiveQuestionId = questions.some((q) => q.id === activeQuestionId)
    ? activeQuestionId
    : (questions[0]?.id ?? null);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const top = visible[0];
        if (!top) return;
        const qid = top.target.getAttribute('data-question-id');
        ...
        {phase === 'answering' && (
          ...
          >
            ...
            <div className="flex-1 min-h-0 flex">
              <div className="w-24 shrink-0 border-r border-gray-100 dark:border-gray-700 p-3 hidden md:block">
                <AnswerSheet
                  questions={questions}
                  answers={answers}
                  phase={phase}
                  resultMap={resultMap}
                  activeQuestionId={safeActiveQuestionId}
